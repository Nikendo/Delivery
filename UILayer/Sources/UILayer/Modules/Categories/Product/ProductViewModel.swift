...
public final class ProductViewModel: ProductViewModelProtocol {
    ...
    public func addToFavorite() {
        Task { [weak addProductToFavoriteUseCase] in
            do {
                let product = self.product.value
                try await addProductToFavoriteUseCase?.execute(product: self.product.value)
    ...
    public func addToCart() {
        Task { [weak addProductToCartUseCase] in
            do {
                let product = self.product.value
                try await addProductToCartUseCase?.execute(product: product)
            } catch {
                self.error.value = error
            }
        }
